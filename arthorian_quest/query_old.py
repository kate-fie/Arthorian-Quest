    ...
    def retrieve(self, query: str, dbnames: List[str], search_type='SMARTS', length=1_000):
        ...
        * N_HA: number of heavy atoms
        ...
        if not data.get('recordsTotal', False):
            ...
            return pd.DataFrame()
        matches = pd.DataFrame(data['data'], columns=['arthor.rank', 'arthor.index', 'smiles', 'identifier', 'arthor.source'])
        ...
        # tabs?
        matches['arthor.source'] = matches['arthor.source'].apply(lambda x: x.replace('\t', '')) # remove tabs
